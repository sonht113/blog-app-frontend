...
export interface ButtonProps
  ...
  /**
   * Custom color for the button. Can be any CSS color (hex, rgb, var()).
   * - For `default` and `secondary` variants this will set `background-color`.
   * - For `outline` and `ghost` variants this will set the `color` and `border-color`.
   */
  color?: string;
  /**
   * Optional foreground/text color to use when `color` sets a background.
   */
  colorForeground?: string;
  /**
   * Semantic color scheme name. Maps to CSS variables (eg. `var(--primary)`).
   * If `color` is provided it takes precedence over `colorScheme`.
   */
  colorScheme?:
    | "primary"
    | "secondary"
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    // Resolve color value: explicit `color` prop takes precedence,
    // otherwise map `colorScheme` to CSS variable.
    ...
    const colorValue = color ?? (colorScheme ? schemeMap[colorScheme] : undefined);

    // Inline style when user provides a custom color or colorScheme
    const customStyle: React.CSSProperties = {};
    if (colorValue) {
      if (variant === "outline" || variant === "ghost" || variant === "link") {
        // set text and border color
        customStyle.color = colorValue;
        if (variant === "outline") customStyle.borderColor = colorValue;
      } else {
        // default/secondary/destructive: set background and optional foreground
        customStyle.backgroundColor = colorValue;
        customStyle.color = colorForeground || "var(--primary-foreground)" || "#ffffff";
      }
    }
